...
export const HeroComparison: React.FC<HeroComparisonProps> = ({ realitySrc, catRealitySrc, dogSrc, catSrc }) => {
  // Use a ref for the slider position to avoid re-renders during animation/interaction
  ...
  // Ensure DOM stays in sync with ref after React renders (prevent snap-back)
  useLayoutEffect(() => {
    updateSlider(positionRef.current);
  });
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      // Adjust speed for 60fps
      // Original: 0.5 per 50ms = 10 units/sec
      // New: 0.16 units/frame at 60fps = 9.6 units/sec
      const step = 0.16;
