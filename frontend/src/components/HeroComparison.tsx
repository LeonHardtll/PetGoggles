...
export const HeroComparison: React.FC<HeroComparisonProps> = ({ realitySrc, catRealitySrc, dogSrc, catSrc }) => {
  // Performance: Use refs for animation to avoid re-renders (60fps vs 20fps react updates)
  ...
  useEffect(() => {
    ...
    const animate = (time: number) => {
      ...
      // Protect against large deltas (e.g. tab switching)
      if (delta > 100) {
          lastTime = time;
      ...
      // Target 60fps equivalent speed (previously 0.5 per 50ms = 10 units / sec)
      // 0.5 unit / 50ms = 0.01 unit / ms
      const step = 0.01 * delta;

      let next = sliderValueRef.current + (step * direction);
  ...
  return (
    <div className="relative w-full max-w-[500px] mx-auto lg:mx-0 select-none group">
      ...
      <div 
        ...
        aria-valuenow={50}
        onKeyDown={(e) => {
            let val = sliderValueRef.current;
            if (e.key === 'ArrowLeft') val -= 5;
