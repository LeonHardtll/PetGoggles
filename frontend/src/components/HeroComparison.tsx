...
export const HeroComparison: React.FC<HeroComparisonProps> = ({ realitySrc, catRealitySrc, dogSrc, catSrc }) => {
  // Performance optimization: Using refs for animation to avoid re-renders
  ...
  // Helper to update the DOM directly
  const updateSlider = useCallback((percentage: number) => {
    sliderPosition.current = percentage;
  ...
  const handleKeyDown = useCallback((e: React.KeyboardEvent) => {
    let newPos = sliderPosition.current;
    if (e.key === 'ArrowLeft') {
  ...
  useEffect(() => {
    ...
    let direction = 1;
    // Check initial direction based on position?
    // The original logic was: if (next > 70) direction = -1; if (next < 30) direction = 1;
    // We can just start assuming 1, or derive from current pos if we wanted to be fancy, but 1 is fine.
    ...
    const animate = (time: number) => {
      ...
      if (delta >= 50) { // Update every ~50ms to match original speed
        let current = sliderPosition.current;
        const next = current + (0.5 * direction);
      ...
      >
        ...
        {/* Layer 2: Pet Vision (Left side, clipped) */}
         <div 
            ...
            style={{ width: `${sliderPosition.current}%` }}
        >
             <img 
        ...
        <div 
            ...
            className="absolute top-0 bottom-0 w-1 bg-white cursor-col-resize z-20 shadow-[0_0_10px_rgba(0,0,0,0.3)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-white"
            style={{ left: `${sliderPosition.current}%` }}
        >
