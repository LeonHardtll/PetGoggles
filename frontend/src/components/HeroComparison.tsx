...
export const HeroComparison: React.FC<HeroComparisonProps> = ({ realitySrc, catRealitySrc, dogSrc, catSrc }) => {
  // Use ref for position to avoid re-renders during animation/drag
  ...
  const updateVisuals = (percentage: number) => {
    ...
    if (overlayRef.current) overlayRef.current.style.width = `${percentage}%`;
  ...
  // Optimized Auto-sweep animation
  useEffect(() => {
    ...
    const animate = (time: number) => {
      const delta = Math.min(time - lastTime, 64); // Cap delta at ~60ms
      lastTime = time;

      // Speed: 0.5% per 50ms = 0.01% per ms
      const speed = 0.01;
      ...
      >
        ...
        <div className="absolute top-4 right-4 bg-black/50 text-white text-xs font-bold px-2 py-1 rounded backdrop-blur-md transition-all">
         ...
         <div 
            ...
            style={{ width: `${positionRef.current}%` }}
        >
             <img 
