...
export const HeroComparison: React.FC<HeroComparisonProps> = ({ realitySrc, catRealitySrc, dogSrc, catSrc }) => {
  // OPTIMIZATION: Use ref instead of state for high-frequency updates to prevent re-renders
  ...
  const updateSlider = useCallback((percentage: number) => {
    ...
    if (clipContainerRef.current) {
      clipContainerRef.current.style.width = `${clamped}%`;
    }
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (changed) {
      e.preventDefault();
      updateSlider(newPos);
      setIsFocused(true); // Ensure focus state is active
    }
  };
  ...
  useEffect(() => {
    ...
    const speed = 0.01; // Speed in percentage per ms (approx 10% per second)
    ...
    <div className="relative w-full max-w-[500px] mx-auto lg:mx-0 select-none group">
      ...
      >
        ...
        <div 
            ...
            className="absolute top-0 bottom-0 w-1 bg-white cursor-col-resize z-20 shadow-[0_0_10px_rgba(0,0,0,0.3)] outline-none focus-visible:ring-2 focus-visible:ring-orange-500"
            style={{ left: `50%` }}
