...
export const HeroComparison: React.FC<HeroComparisonProps> = ({ realitySrc, catRealitySrc, dogSrc, catSrc }) => {
  // Optimization: Use refs for high-frequency updates (slider move) to avoid re-renders
  ...
  const updateSlider = (percentage: number) => {
    ...
    if (clipRef.current) {
      clipRef.current.style.width = `${percentage}%`;
    }
  ...
  // Handle container resize to keep inner image correct
  useEffect(() => {
    const updateWidth = () => {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      let next = positionRef.current + (0.5 * directionRef.current);
      if (next > 70) {
          next = 70;
    ...
    <div className="relative w-full max-w-[500px] mx-auto lg:mx-0 select-none group">
      ...
      >
        ...
        <div 
            ...
            style={{ left: '50%' }} // Initial position
        >
