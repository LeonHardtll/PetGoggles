...
export const HeroComparison: React.FC<HeroComparisonProps> = ({ realitySrc, catRealitySrc, dogSrc, catSrc }) => {
  // OPTIMIZATION: Use refs for direct DOM manipulation to avoid re-renders on every frame (slider movement)
  ...
  // Auto-sweep animation when not hovering using requestAnimationFrame for 60fps smoothness
  useEffect(() => {
    if (isHovering) return;
    
    let animationFrameId: number;
    let direction = 1;
    // We can't easily persist direction across re-renders of the effect without a ref,
    // but resetting direction on hover end is acceptable behavior.
    ...
    const animate = (time: number) => {
      ...
      const speed = 0.01; // 0.5 units per 50ms = 0.01 units per ms
      let next = sliderPositionRef.current + (speed * cappedDelta * direction);
